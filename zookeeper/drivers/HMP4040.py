...
class HMP4040(VISA_Instrument): 
    ...
    def connect(self):
        ...
        super().connect()
        for i in range(1, config['NCHANNELS']+1):
            self[i].apply(voltage = 0.0, current = 0.0)
            self.output = 'OFF'
    ...
    def __getitem__(self, key : int):
        if key in range(1, config['NCHANNELS']+1):
            ...
            raise ValueError(f"Specified channel {key} not allowed: {config['NCHANNELS']} on device")
